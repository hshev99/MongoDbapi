...
class MongoDbApi {
    ...
    public $mongo;
    private $host = 'mongo.tuodui.com';
    private $port = 27017;
    private $db;
    public $dbname = 'gnss_platform';
    private $table = 'tmn_location';
    ...
    public function getNext(){
        try {
            if ($nextValue = $this->db->getNext()) {
                return $nextValue;
            } else {
                throw new MongoException('获取下一个值失败');
            }
    ...
    public function __construct($host = NULL, $port = NULL, $dbname = NULL, $table = NULL) {
        ...
        $this->table = $table;
        $this->mongo = new MongoClient('mongodb://' . $this->host . ':' . $this->port);
        if ($this->getVersion() >= '0.9.0') {
//            var_dump($this->getVersion());die;//1.4.5
     ...
     * @return Mongo|null
     */
    public static function getInstance(Array $linkMessage = array()){
        if(!(self::$instanceof instanceof self)){
            self::$instanceof = new self($linkMessage['host'], $linkMessage['port'], $linkMessage['dbname'], $linkMessage['table']);
        }
        ...
        if (NULL === $where) {
            try {
                if ($result = $this->db->findOne()) {
                    ...
                    return false;
                    throw new MongoException('查找数据失败1');
                }
            } catch (MongoException $e) {
                $this->throwError($e->getMessage());
            }
            ...
            try {
//                $result = $this->db->findOne($where);
//                    return $result;
                if ($result = $this->db->findOne($where)) {
                    return $result;
                } else {
                    return false;
                    throw new MongoException('查找数据失败2');
                }
            } catch (MongoException $e) {
                $this->throwError($e->getMessage());
            }
    ...
    public function updateDb($where, $newdata){

        if (empty($newdata)) {
            $this->throwError('修改的数据不能为空！');
        }
        //修改数据信息
        try {
            if (!$result=$this->db->update($where,array('$set'=>$newdata))) {
                throw new MongoException('插入数据失败');
